...
<div id="actions_required" class="islemag-tab-pane">
	...
	if( !empty($islemag_required_actions) ):
		...
		foreach( $islemag_required_actions as $islemag_required_action_key => $islemag_required_action_value ):
			if(@$islemag_show_required_actions[$islemag_required_action_value['id']] === false) continue;
			if(@$islemag_required_action_value['check']) continue;
			?>
			<div class="islemag-action-required-box">
				...
				<h4><?php echo $islemag_required_action_key + 1; ?>. <?php if( !empty($islemag_required_action_value['title']) ): echo $islemag_required_action_value['title']; endif; ?></h4>
				<p><?php if( !empty($islemag_required_action_value['description']) ): echo $islemag_required_action_value['description']; endif; ?></p>
				<?php
					if( !empty($islemag_required_action_value['plugin_slug']) ):
						?><p><a href="<?php echo esc_url( wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin='.$islemag_required_action_value['plugin_slug'] ), 'install-plugin_'.$islemag_required_action_value['plugin_slug'] ) ); ?>" class="button button-primary"><?php if( !empty($islemag_required_action_value['title']) ): echo $islemag_required_action_value['title']; endif; ?></a></p><?php
					endif;
				?>
