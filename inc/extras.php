    ...
    function islemag_footer()
    {
        ...
        <div class="col-md-4 col-md-pull-8 powerdby">
            <?php printf(
                ...
                sprintf('<a href="https://themeisle.com/themes/islemag/" rel="nofollow">%s</a>',
                    esc_html__('Islemag', 'islemag')),
...
if (!function_exists('islemag_comment_action')) {
    function islemag_comment_action($args, $comment, $depth, $add_below)
    {
        ...
        <div class="comment-author vcard">
            ...
            <?php printf(__('<h4 class="media-heading">%s</h4><span class="comment-date">(%2$s - %3$s)</span>',
                'islemag'), get_comment_author_link(), get_comment_date(),
                get_comment_time()); ?><?php edit_comment_link(__('(Edit)', 'islemag'), '  ', ''); ?>
            <div class="reply pull-right reply-link"> <?php comment_reply_link(array_merge($args,
                    array('add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth']))); ?> </div>
        </div>
