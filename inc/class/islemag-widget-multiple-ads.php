        ...
        for( $i = 1 ; $i < 7 ; $i++ ){
          ...
          if( !empty($instance[$url]) ){
            if( !empty( $instance[$link] ) ){
              echo '<div class="col-md-6 '.( $i % 2 == 0 ? 'no-padding-left' : 'no-padding-right' ).'"> <a href="'.$instance[$link].'" target="'.( !empty( $instance['new_tab'] ) && $instance['new_tab'] == 'on' ? '_blank' : '' ).'"><img src="'.$instance[$url].'" alt="'.( !empty( $instance[$title_alt] ) ? $instance[$title_alt] : '' ).'"/></a></div>';
            } else {
              echo '<div class="col-md-6 '.( $i % 2 == 0 ? 'no-padding-left' : 'no-padding-right' ).'"> <img src="'.$instance[$url].'" alt="'.( !empty( $instance[$title_alt] ) ? $instance[$title_alt] : '' ).'"/></div>';
            }
          }
        }
...
?>
  ...
  <p>
    <label for="<?php echo $this->get_field_id('widget_title'); ?>"><?php _e('Title','islemag'); ?></label><br />
    <input type="text" name="<?php echo $this->get_field_name('widget_title'); ?>" id="<?php echo $this->get_field_id('widget_title'); ?>" value="<?php if( !empty($instance['widget_title']) ): echo $instance['widget_title']; endif; ?>" class="widefat" />
  </p>
  ...
  <p>
    ...
    <label for="<?php echo $this->get_field_id('new_tab'); ?>"><?php _e('Open in new tab','islemag'); ?></label>
  </p>
  ...
  <h3><?php esc_html_e('Advertisement ','islemag'); echo $i; ?> </h3>
	<p>
		<label for="<?php echo $this->get_field_id($title_alt); ?>"><?php _e('Alt Title','islemag'); ?></label><br />
		<input type="text" name="<?php echo $this->get_field_name($title_alt); ?>" id="<?php echo $this->get_field_id($title_alt); ?>" value="<?php if( !empty($instance[$title_alt]) ): echo $instance[$title_alt]; endif; ?>" class="widefat" />
	</p>

	<p>
    <label for="<?php echo $this->get_field_id($link); ?>"><?php _e('Link','islemag'); ?></label><br />
	...
	<p>
    <label for="<?php echo $this->get_field_name( $url ); ?>"><?php _e( 'Image:', 'islemag' ); ?></label>
    <input name="<?php echo $this->get_field_name( $url ); ?>" id="<?php echo $this->get_field_id( $url ); ?>" class="widefat" type="text" size="36"  value="<?php if( !empty($instance[$url]) ): echo esc_url( $instance[$url] ); endif; ?>" />
